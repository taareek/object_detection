...
class YOLOV1Loss(nn.Module):
    """
    Loss module for YOLOV1 is combined of the following componenets:
    1. Localization Loss for responsible predictor boxes
    2. Objectness Loss for responsible predictor boxes
    2. Objectness Loss for non-responsible predictor boxes of cells assigned with other objects
    ...
    def forward(self, preds, targets, use_sigmoid=False):
        ...
        # we will use these for converting x_center_offset/y_center_offset
        # values to x1/y1/x2/y2 (normalized 0-1)
        # S celss = 1 => each cell adds 1/S pixels of shift 
        ...
        # target_boxes -> (Batch_size, S, S, B, 5) : ground truth n
        ...
        # Maximum iou val / max iou idx -> (Batch_size, S, S, 1)
        ...
        # only keep loses from cells with object assigned
        cls_mse = (obj_indicator * cls_mse).sum()

        #########################
        # Objectiveness Loss -> For responsible predictor boxes
        #########################
        # indicator is now object cells * is_best_box
